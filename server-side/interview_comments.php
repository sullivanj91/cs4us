...
require_once('orm/Interview_Comment.php');
...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    if ((count($path_components) >= 2) &&
        ...
        $comment = Interview_Question::findByID($question_id);

        if ($comment == null) {
            // Question not found.
            header("HTTP/1.0 404 Not Found");
            print("Interview_Comment id: " . $question_id . " not found.");
            exit();
        }
        ...
        print(json_encode(Interview_Comment::findByQuestionID($question_id)));
        exit();
    ...
    header("Content-type: application/json");
    print(json_encode(Interview_Comment::getAll()));
    exit();
    ...
    if ((count($path_components) >= 2) &&
        ...
        $comment = Interview_Comment::findByID($comment_id);

        if ($comment == null) {
            // Question not found.
            header("HTTP/1.0 404 Not Found");
            print("Interview_Comment id: " . $comment_id . " not found while attempting update.");
            exit();
        }
        ...
        // Creating a new Interview_Comment item
        ...
        $new_comment = Interview_Comment::create($question_id, $user, $comment_text);

        // Report if failed
        if ($new_comment == null) {
            header("HTTP/1.0 500 Server Error");
            print("Server couldn't create new Interview_Comment.");
            exit();
        }
